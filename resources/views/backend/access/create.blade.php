            ...
            <div class="col-lg-3">
                @if (count($roles) > 0)
                    ...
                    @foreach($roles as $role)
                        <input type="radio" value="{{$role->id}}" name="assignees_roles" id="role-{{$role->id}}" checked /> <label for="role-{{$role->id}}">{!! $role->name !!}</label> <a href="#" data-role="role_{{$role->id}}" class="show-permissions small">(<span class="show-hide">Show</span> Permissions)</a><br/>
