...
class GAuthController extends Controller
{
 ...
 $validator = Validator::make($input, [ 
    ...
    if($oldemail ==trim($input['uemail']) && $emailconfirmed== 0){ //condition for email and confirmed=0 start if
    ...
    }elseif ($oldemail ==trim($input['uemail']) && $emailconfirmed== 1) {//condition for email and confirmed=1start elseif
      return response()->json([
          "response"=>[
          [
            "rcode" => 1,
      ...
      if($oldemail != trim($input['uemail']) && $oldemail='null'){   //condition for email and confirmed is not present previous start if
        $User = new User;
        $Lang =new Language;
        ...
        Mail::send('email.verify', ['confirmation_code' =>$confirmation_code, 'fname'=>$input['ufirstname'], 'lname'=>$input['ulastname']], function($message)
    {
        $message
            ->to(Input::get('uemail'))            
    ...
    $User->password           =Hash::make(trim($input['upassword']));    
    ...
    //condition for message end    
      ...
      // If validation falis redirect back to login.
       ...
       // var_dump(Auth::attempt('email'=>$request->uemail)); die();  
        ...
        }elseif ($oldemail ==trim($input['uemail']) && $emailconfirmed== 0) {
         return response()->json([
            "response"=>[
                [
                "rcode" => 1,
        ...
        }elseif(($oldemail ==trim($input['uemail'])) && (($emailconfirmed== 1) && (!Hash::check(trim($input['upassword']) ,$password )))) {
           return response()->json([
           "response"=>[
                [
                "rcode" => 1,
...
public function getApiChangePassword(Request $request){
   ...
   // Auth::logout();
    ...
    $oldpassword= trim($input['uoldpassword']);
    //var_dump($oldpassword);die();
    $epassword = User::where('id', $input['uid'])->value('password');
    //$oldemail=  User::where('email', trim($input['uemail']))->value('email');
    //$emailconfirmed=  User::where('email', trim($input['uemail']))->value('confirmed');
   // $password=  User::where('email', trim($input['uemail']))->value('password');
    ...
    if (empty($input['uid']) && $role_id==1) { 
         ...
         return response()->json([
          ...
          try {
            ...
            ->update(['password' => Hash::make(trim($input['unewpassword']))]);             
             return response()->json([
            "response"=>[
                [
                "rcode" => 0,
...
//$extension = $uploadedfilename->getClientOriginalExtension();
            ...
            $path ='uploads/'.$input['uid'].'/original/profilePic/';
            $pathresize ='uploads/'.$input['uid'].'/resize/profilePic/';
                if(!File::exists($path) && !File::exists($pathresize)) {
              File::makeDirectory('uploads/'.$input['uid'].'/original/profilePic/', 0775, true);
              File::makeDirectory('uploads/'.$input['uid'].'/resize/profilePic/', 0775, true);
              $target_path = 'uploads/'.$input['uid'].'/original/profilePic/'. $new. '_'. $file;
              $target_pathsmall ='uploads/'.$input['uid'].'/resize/profilePic/'. $random.'_'. $file;
           ...
           $Profiles= DB::table('profiles')         
            ...
            ->update(['profileImg' => 'http://www.guidenp.com/uploads/'.$input['uid'].'/original/profilePic/'.$new.'_'.$file,
              'profilesmallImg' =>'http://www.guidenp.com/uploads/'.$input['uid'].'/resize/profilePic/'.$random.'_'.$file]);
...
public function getApiAddCertificate(Request $request){
 ...
 //condtion for message
    ...
    if (!$validator->fails()){ 
      ...
      if (empty($input['uid']) && ($role_id==1 && $role_id==3)) { 
          ...
          // move_uploaded_file($uploadedfilename1, $target_pathsmall);
              $path ='uploads/'.$input['uid'].'/original/license/';
              $pathresize ='uploads/'.$input['uid'].'/resize/license/';
                if(!File::exists($path) && !File::exists($pathresize)) {
              File::makeDirectory('uploads/'.$input['uid'].'/original/license/', 0775, true);
              File::makeDirectory('uploads/'.$input['uid'].'/resize/license/', 0775, true);
              $target_path = 'uploads/'.$input['uid'].'/original/license/'. $new. '_'. $file;
              $target_pathsmall ='uploads/'.$input['uid'].'/resize/license/'. $random.'_'. $file;
 ...
 // return "test1";
          ...
          $Gallery->path = 'http://www.guidenp.com/uploads/'.$input['uid'].'/original/license/'. $new.'_'. $file;
          $Gallery->imagesmall  = 'http://www.guidenp.com/uploads/'.$input['uid'].'/resize/license/'. $random. '_'. $file;
    ...
    if (!$validator->fails()){  
      ...
      if (empty($input['uid']) && ($role_id==1 && $role_id==3)) { 
          ...
          try {
            ...
            // move_uploaded_file($uploadedfilename1, $target_pathsmall);
              $path ='uploads/'.$input['uid'].'/original/gallery/';
              $pathresize ='uploads/'.$input['uid'].'/resize/gallery/';
                if(!File::exists($path) && !File::exists($pathresize)) {
              File::makeDirectory('uploads/'.$input['uid'].'/original/gallery/', 0775, true);
              File::makeDirectory('uploads/'.$input['uid'].'/resize/gallery/', 0775, true);
              $target_path = 'uploads/'.$input['uid'].'/original/gallery/'. $new. '_'. $file;
              $target_pathsmall ='uploads/'.$input['uid'].'/resize/gallery/'. $random.'_'. $file;
...
if($move){
  $target_path = 'uploads/'.$input['uid'].'/original/gallery/'. $new. '_'. $file;
            ...
            $Gallery->user_id = $input['uid'];
            //$Gallery->caption =$input['caption'];       
            $Gallery->path = 'http://www.guidenp.com/uploads/'.$input['uid'].'/original/gallery/'. $new . '_' .$file;
            $Gallery->imagesmall  = 'http://www.guidenp.com/uploads/'.$input['uid'].'/resize/gallery/'. $random . '_'. $file;
            $Gallery->type ='image';   
            $Gallery->save();
...
//var_dump('a'); die();
    ...
    if (!$validator->fails()){
      ...
      $oldemail=  User::where('email', trim($input['uemail']))->value('email');
      $oldemailid=  User::where('email', trim($input['uemail']))->value('id');  
      $role_id =DB::table('assigned_roles')->where('user_id' , $oldemailid)->value('role_id');
        if ($oldemail !=trim($input['uemail']) && empty($oldemail)) { 
          return response()->json([
            "response"=>[
                [
                "rcode" => 1,
